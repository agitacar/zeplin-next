...
export default function Home() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      if (data.user) {
        const { data: profile } = await supabase.from('profiles').select('email, full_name').eq('id', data.user.id).single();
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 40px', background: '#fff', borderBottom: '1px solid #DDE8DD', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        {userLoading ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#F4F7F4', padding: '6px 14px 6px 6px', borderRadius: 24 }}>
              <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#2A7A4B', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>
                {(user.full_name || user.email || 'U').charAt(0).toUpperCase()}
              </div>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#141A14' }}>{user.full_name || user.email?.split('@')[0]}</span>
            </div>
            <button onClick={handleLogout} style={{ background: 'transparent', color: '#8A9E8A', border: '1px solid #DDE8DD', padding: '8px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Cikis</button>
          </div>
        ) : (
          <a href="/giris" style={{ background: '#2A7A4B', color: '#fff', border: 'none', padding: '9px 20px', borderRadius: 24, fontSize: 13, fontWeight: 700, cursor: 'pointer', textDecoration: 'none' }}>Uye Ol</a>
        )}
      </nav>
