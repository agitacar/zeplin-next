...
export default function Home() {
  ...
  const slides = [
    {
      ...
      tag: 'Teknoloji',
      title: 'En iyi fiyat,\nbir tik uzaginda.',
      sub: 'Telefon, laptop, kulaklik - yuzlerce magazayi aninda karsilastir.',
      btn: 'Telefon ara',
    },
    {
      bg: 'linear-gradient(135deg, #1a6b3c, #2d9e6b, #1a6b3c)',
      tag: 'Seyahat',
      title: 'Ucusunu karsilastir,\nen ucuzunu bul.',
      sub: 'Ucak bileti, otel, arac kiralama - tum seyahat platformlarini tek yerde gor.',
      btn: 'Seyahate bak',
    },
    {
      ...
      tag: 'Gunun Firsati',
      title: 'Fiyat dustu,\nalarm caldi!',
      sub: 'Istedigin urunun fiyati dusunce seni haberdar edelim.',
      btn: 'Alarm kur',
    },
    {
      ...
      tag: 'Ev & Yasam',
      title: 'Evin icin\nen iyi secim.',
      sub: 'Beyaz esyadan mobilyaya, tum ev urunlerini karsilastir.',
      btn: 'Kesfet',
    },
  ];
  ...
  const goTo = (n: number) => {
    setCurrent((n + slideCount) % slideCount);
  };
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#F4F7F4', minHeight: '100vh', color: '#141A14' }}>
      ...
      <div style={{ padding: '20px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 40, padding: '20px 40px', background: '#fff', border: '1px solid #DDE8DD', borderRadius: 16 }}>
          {[
            { num: '2.4M+', label: 'Urun takip ediliyor' },
            { num: '150+', label: 'Magaza karsilastiriliyor' },
            { num: '142M TL', label: 'Kullanici tasarrufu' },
            { num: '98%', label: 'Yorum dogrulugu' },
          ].map((stat, i) => (
      ...
      <div style={{ margin: '0 40px 40px', background: '#fff', border: '1px solid #DDE8DD', borderRadius: 16, padding: '22px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap' }}>
          {[
            { icon: '🔔', text: 'Fiyat alarmi kur' },
            { icon: '📈', text: 'Fiyat gecmisini gor' },
